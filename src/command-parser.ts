...
export class CommandParser {
  ...
  private intentPatterns = [
    // File operations
    {
      pattern: /(?:list|show|see|display).*(?:files|directories|folder)/i,
      commands: ['ls -la', 'ls -lah', 'find . -type f'],
      category: 'files'
    },
    {
      pattern: /(?:create|make|new).*(?:file|directory|folder)/i,
      commands: ['touch', 'mkdir'],
      category: 'files'
    },
    {
      pattern: /(?:delete|remove|rm).*(?:file|directory)/i,
    ...
    {
      pattern: /(?:find|search|locate).*file/i,
      commands: ['find . -name', 'locate', 'grep -r'],
      category: 'files'
    },

    // System information
    {
      pattern: /(?:system|os).*(?:info|information|details)/i,
      commands: ['uname -a', 'cat /etc/os-release', 'hostnamectl'],
      category: 'system'
    },
    {
      pattern: /(?:disk|storage).*(?:space|usage)/i,
      commands: ['df -h', 'du -sh *', 'lsblk'],
      category: 'system'
    },
    {
      pattern: /(?:memory|ram).*usage/i,
      commands: ['free -h', 'cat /proc/meminfo'],
      category: 'system'
    },
    {
      pattern: /(?:cpu|processor).*(?:usage|info)/i,
      commands: ['top', 'htop', 'cat /proc/cpuinfo'],
      category: 'system'
    },
    {
      pattern: /(?:what|which).*(?:processes|running)/i,
      commands: ['ps aux', 'top', 'htop'],
      category: 'system'
    },
    {
      pattern: /(?:running|active).*(?:processes|services)/i,
      commands: ['ps aux', 'systemctl list-units --type=service'],
      category: 'system'
    },
    ...
    // Network operations
    {
      pattern: /(?:network|internet).*(?:connection|connectivity)/i,
      commands: ['ping -c 4 google.com', 'curl -I google.com'],
      category: 'network'
    },
    {
      pattern: /(?:network|open).*(?:ports|connections)/i,
      commands: ['netstat -tuln', 'ss -tuln'],
      category: 'network'
    },
    {
      pattern: /(?:ip|network).*(?:address|config)/i,
      commands: ['ip addr show', 'ifconfig'],
      category: 'network'
    },
    {
      pattern: /(?:download|fetch|get).*(?:file|url)/i,
      commands: ['wget', 'curl -O'],
      category: 'network'
    },

    // Service management
    {
      pattern: /(?:start|stop|restart|status).*service/i,
      commands: ['systemctl start', 'systemctl stop', 'systemctl restart', 'systemctl status'],
      category: 'services'
    },
    {
      pattern: /(?:docker|container)/i,
      commands: ['docker ps', 'docker images', 'docker logs'],
      category: 'docker'
    },
    {
      pattern: /(?:git|repository)/i,
      commands: ['git status', 'git log --oneline', 'git branch'],
      category: 'git'
    },

    // Package management
    {
      pattern: /(?:install|update|upgrade).*(?:package|software)/i,
      commands: ['apt update && apt upgrade', 'yum update', 'npm install'],
      category: 'packages'
    },
    {
      pattern: /(?:search|find).*package/i,
      commands: ['apt search', 'yum search', 'npm search'],
      category: 'packages'
    },

    // Log viewing
    {
      pattern: /(?:view|show|check|read).*(?:logs|log)/i,
      commands: ['tail -f /var/log/syslog', 'journalctl -f', 'dmesg'],
      category: 'logs'
    },

    // Text operations
    {
      pattern: /(?:view|show|read|cat).*(?:file|content)/i,
      commands: ['cat', 'less', 'head', 'tail'],
      category: 'text'
    },
    {
      pattern: /(?:search|grep|find).*(?:text|string|pattern)/i,
      commands: ['grep -r', 'find . -type f -exec grep'],
      category: 'text'
    },
