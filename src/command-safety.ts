...
function hasShortFlag(words: string[], flag: string): boolean {
  return words.some((word) => word.startsWith('-') && !word.startsWith('--') && word.slice(1).includes(flag));
}
...
function assessSegment(segment: string): RiskAssessment {
  ...
  const recursive = hasShortFlag(args, 'r') || hasShortFlag(args, 'R') || args.includes('--recursive');
  ...
  if (command === 'dd' && args.some((arg) => arg.startsWith('of=') && BLOCK_DEVICE_RE.test(arg.slice(3)))) {
  ...
  if (command === 'rm' && (recursive || forced)) return assessment('destructive', 'Forced or recursive deletion');
  if (command === 'dd') return assessment('destructive', 'Low-level data copying can overwrite data');
  if (['shutdown', 'reboot', 'halt', 'poweroff'].includes(command)) return assessment('destructive', 'Command stops or restarts the system');
  if (command === 'systemctl' && ['stop', 'disable', 'mask'].includes(args[0] ?? '')) return assessment('destructive', 'Command disables or stops a service');
  if (['kill', 'pkill', 'killall'].includes(command)) return assessment('destructive', 'Command terminates processes');
  if ((command === 'iptables' && (args.includes('-F') || args.includes('--flush'))) || (command === 'nft' && args.includes('flush'))) return assessment('destructive', 'Command flushes firewall rules');
  if (command === 'truncate') return assessment('destructive', 'Command truncates file data');
  if (['userdel', 'groupdel'].includes(command)) return assessment('destructive', 'Command deletes an account or group');
  if (command === 'crontab' && args.includes('-r')) return assessment('destructive', 'Command removes a crontab');
  if (command === 'docker' && ((args[0] === 'system' && args[1] === 'prune') || ['rm', 'rmi'].includes(args[0] ?? ''))) return assessment('destructive', 'Command removes Docker resources');
  if (command === 'git' && args[0] === 'push' && (args.includes('--force') || args.includes('-f') || args.includes('--force-with-lease'))) return assessment('destructive', 'Forced Git push can overwrite remote history');
  ...
  if (packageManagers.test(command) && args.some((arg) => packageActions.test(arg))) return assessment('caution', 'Command changes installed packages');
...
export function assessCommandRisk(command: string): RiskAssessment {
  const masked = maskQuotedText(command).replace(/[\t\r ]+/g, ' ').trim();
